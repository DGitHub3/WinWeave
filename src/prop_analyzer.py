...
from src.factors.usage        import usage_multiplier
...
_LATEST_SEASON_CACHE: list = []

def _latest_db_season() -> int:
    """Latest NFL season present in props — replaces hardcoded 2024."""
    if _LATEST_SEASON_CACHE:
        return _LATEST_SEASON_CACHE[0]
    conn = get_connection()
    try:
        row = conn.execute("SELECT MAX(season) FROM props").fetchone()
        season = row[0] or 2024
        _LATEST_SEASON_CACHE.append(season)
        return season
    finally:
        conn.close()
...
def analyze_prop(
    ...
    # v4 FIX: previous fallback was `or opponent` — if the roster
    # lookup missed, the OPPONENT was silently treated as the
    # player's own team, so roster health, coaching tendency, pace,
    # weather, and officials were all computed for the wrong club.
    # Now: no team -> neutral multipliers, honestly labeled.
    team = get_player_team(player_name, season)
    if team:
        r_mult, r_details = roster_multiplier(player_name, team,
                                              season=season, week=week)
    else:
        r_mult, r_details = 1.0, {"injury_status":
                                  "team unresolved — neutral"}

    # ── Signal 6: coaching tendency + pace ────────────────────
    # v4 FIX: was `season or 2024` — a hardcoded year that silently
    # went stale. Now anchored to the latest season in the database.
    c_season = season or _latest_db_season()
    if team:
        ...
        c_mult, c_desc = 1.0, "team unresolved — neutral"

    # ── Signal 7: weather ─────────────────────────────────────
    weather = get_weather_for_team_game(team, season, week) \
        if (team and season and week) else {}
    ...
    referee = get_referee_for_team_week(team, season, week) \
        if (team and season and week) else None
    o_mult = official_multiplier(referee, stat)
    o_desc = describe_crew(referee)

    # ── Signal 9 (v3.5): usage / star-reliability ─────────────
    u_season = season or _latest_db_season()
    u_mult, u_desc, u_score = usage_multiplier(
        player_name, team, position, stat, u_season)

    # ── Book probabilities FIRST (v4: needed as calibration anchor) ─
    ...
    # v4: empirical-Bayes shrinkage on the hit-rate signal — a raw
    # 5-for-6 streak (83%) is pulled toward the market base rate in
    # proportion to how little data supports it.
    hr_signal = shrink_hit_rate(hr_signal * sample, sample, no_vig_prob)

    # ── Combine all 8 signals ─────────────────────────────────
    raw_prob = combine_all_signals(
    ...
    # ── v4 CALIBRATION: market anchoring + edge cap ───────────
    # The single fix for the 94%-vs-45% class of error shown in the
    # 07/09-07/10 tracked exports. See src/calibration.py.
    cal = calibrate(raw_prob, no_vig_prob, sample)
...
def print_full_analysis(result: PropAnalysis):
    ...
    print(f"  |- Coaching/pace: {result.coaching_mult:.2f}x  (w  9%)")
    print(f"  |- Usage:         {result.usage_mult:.2f}x  (w 12%)  {result.usage_desc}")
    print(f"  |- Weather:       {result.weather_mult:.2f}x  (w  8%)  {result.weather_desc}")
    print(f"  '- Officials:     {result.official_mult:.2f}x  (w  5%)")

    print(f"\n  VERDICT  —  {ev_label}")
    print(f"  |- Raw model:    {result.raw_model_prob:.1%}  "
          f"(signal blend, pre-anchor)")
    print(f"  |- Market anchor:{result.no_vig_probability:>7.1%}  "
          f"(model weight {result.model_weight:.0%})")
    print(f"  |- TRUE PROB:    {result.true_probability:.1%}"
          f"{'  [edge-capped]' if result.cal_capped else ''}")
